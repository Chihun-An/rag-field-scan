...
이 항목은 9/21 멘토 피드백에서 "에이전트를 여러 개 쓸 때 규칙이 어디까지 적용되는지 정리해 두라"는 지적을 받아 추가했다. Claude Cowork가 사용자 승인을 받아 GitHub Pages 설정을 대신하고, 사용자가 잠든 사이 README·worklog를 대신 작성한 일(9/20 00:22)이 있었다. 에이전트가 Claude Code 하나가 아니라 여러 개로 늘어나면서, 같은 작업 규칙이 모든 에이전트에 똑같이 적용되는지를 분명히 할 필요가 생겼다. 그래서 AGENTS.md에 "적용 범위(에이전트가 여러 개일 때)" 항목을 추가해, 파일 작업은 에이전트가 하되 commit·push·설정 변경·배포는 사람이 하고, 사람이 건건이 승인한 경우에만 에이전트가 대신할 수 있으며 그 사실을 worklog.md에 남기도록 못박았다.
